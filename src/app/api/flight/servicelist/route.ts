...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json() as ServiceListRequest
    console.log('🍽️ ServiceList proxy received request:', body)
    ...
    const responseData: unknown = await new Promise((resolve, reject) => {
      const url = new URL(API_BASE_URL.replace(/\/$/, '') + '/ServiceList')
    ...
    console.log('✅ ServiceList response received:', isSuccess ? 'Success' : 'Failed')

    return NextResponse.json(responseData, { status: 200 })
  } catch (error) {
    console.error('❌ ServiceList proxy error:', error)
    return NextResponse.json(
      {
        success: false,
        error: {
          errorCode: 'PROXY_ERROR',
          errorMessage: error instanceof Error ? error.message : 'Failed to fetch service list',
        },
      },
      { status: 500 }
    )
