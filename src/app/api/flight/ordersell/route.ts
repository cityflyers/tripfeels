...
export async function POST(request: NextRequest) {
  try {
    const body = (await request.json()) as OrderSellRequest
    ...
    const url = new URL(API_BASE_URL.replace(/\/$/, '') + '/OrderSell')
    ...
    return new Promise<NextResponse>((resolve) => {
      ...
      req.on('error', (error) => {
        console.error('OrderSell API error:', error)
    ...
    console.error('OrderSell route error:', error)
