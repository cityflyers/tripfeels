...
/** OrderCancel request: { "orderReference": "BDF250627784" } */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = new URL(API_BASE.replace(/\/$/, '') + '/OrderCancel')
    ...
    return new Promise<NextResponse>((resolve) => {
      ...
      req.on('error', (err: Error) => {
        console.error('OrderCancel API error:', err)
        resolve(
          NextResponse.json(
    ...
    console.error('OrderCancel route error:', error)
    return NextResponse.json(
