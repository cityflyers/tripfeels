...
export default function BookingsPage() {
  ...
  const handleBookingAction = useCallback(
    async (action: 'view' | 'edit' | 'delete' | 'refresh', booking: Booking) => {
      ...
      switch (action) {
        ...
        case 'delete':
          // Show confirmation and delete booking
          if (
            window.confirm(
              `Are you sure you want to delete booking ${booking.referenceNo}? This action cannot be undone.`,
            )
          ) {
            try {
              const res = await fetch('/api/bookings', {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ orderReference: booking.referenceNo }),
              })

              if (res.ok) {
                // Refresh bookings list
                setBookings((prev) => prev.filter((b) => b.referenceNo !== booking.referenceNo))
                alert('Booking deleted successfully')
              } else {
                const errorData = (await res.json()) as { error?: string }
                setRefError(errorData.error || 'Failed to delete booking')
              }
            } catch (_error) {
              setRefError('Failed to delete booking')
            }
          }
          break
  ...
  return (
    <div className="flex flex-col h-full w-full">
      {/* TabBar and TicketStatusTabs container positioned below header */}
      <div
        className="w-full bg-[var(--tf-surface)] border-b border-[var(--tf-border)]"
      >
        ...
        {activeTab === 0 && (
          <div className="flex flex-col md:flex-row md:items-center md:justify-end gap-2 px-4 md:px-6 py-3 bg-gray-50/50 dark:bg-white/5 border-t border-[var(--tf-border)]">
            <div className="relative flex-1 w-full md:max-w-sm">
              ...
              <Input
                ...
                className="h-8 pl-8 pr-3 text-xs rounded-md border border-[var(--tf-border)] bg-[var(--tf-surface)] shadow-sm focus:ring-2 focus:ring-primary/30 focus:border-primary w-full"
              />
            </div>
            <Button
              variant="outline"
              ...
              className="h-8 gap-1.5 px-3 text-xs font-medium bg-[var(--tf-surface)] border border-[var(--tf-border)] w-full md:w-auto"
            >
      ...
      <main className="flex-1 w-full bg-gray-50/50 dark:bg-neutral-900/20">
        <div className="w-full max-w-[100vw] px-4 md:px-6 py-6">
          {activeTab === 0 && (
            <>
              {bookingsLoading ? (
                <div className="py-12 text-center text-sm text-[var(--tf-text-muted)]">
                  Loading bookings…
                </div>
              ) : (
                <>
